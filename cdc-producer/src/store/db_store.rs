...
use crate::mysql::get_table_columns;
...
impl DbStore {
    ...
    pub fn save_columns(&mut self, db_name: &String, table_name: &String, columns: &Vec<Column>) {
        let table_store = match self.dbs.get_mut(db_name) {
            Some(table_store) => table_store,
            None => {
                self.dbs.insert(db_name.clone(), TableStore::new());
    ...
    #[allow(dead_code)]
    pub fn has_columns(&self, db_name: &String, table_name: &String) -> bool {
    ...
    pub fn clear_columns(&mut self, db_name: &String, table_name: &String) {
