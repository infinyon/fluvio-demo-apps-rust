...
impl BinLogFile {
    pub fn new(base_dir: &PathBuf, file_ref: &String, offset: Option<u64>) -> Result<Self, Error> {
        let path = base_dir.join(file_ref);
        let file_metadata = fs::metadata(&path)?;
        let file_id = get_file_id(&path);
        let file = file_ref.clone();

        return Ok(BinLogFile {
            path: path,
            file: file,
            file_id: file_id,
            offset: offset,
...
pub fn get_file_id(path: &PathBuf) -> i32 {
    let extension = path.extension().unwrap_or(OsStr::new("0"));
...
mod test {
    ...
    fn test_bin_file() {
        ...
        assert_eq!(
            format!("{}", bin_file),
            format!("[000001 -                    0] {:?}", bl_file_path.clone())
        );
