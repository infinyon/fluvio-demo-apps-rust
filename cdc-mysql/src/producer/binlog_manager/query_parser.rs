...
impl fmt::Display for TableOp {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            TableOp::CreateTable(table, columns) => write!(f, "Create Table {} - add columns ({})", table, columns.join(", ")),
            TableOp::AlterTable(table, column_op) => {
                match column_op {
                    ColumnOp::Add(name) => write!(f, "Alter Table {} - add column {}", table, name),
                    ColumnOp::Rename(old_name, new_name) => write!(f, "Alter Table {} - rename column {} to {}", table, old_name, new_name),
            ...
            match statement {
                Statement::CreateTable { name, columns, .. } => {
                    table_ops.push(
                        TableOp::CreateTable(
                            name.to_string(), 
                            columns.iter().map(|x| x.name.to_string()).collect::<Vec<String>>()
                        )
                    )
                }
