...
impl OffsetStore {
    pub fn init(offset_file: &PathBuf) -> Result<OffsetStore, Error> {
        let file = run_block_on(get_or_create_file(offset_file))?;
...
mod test {
    ...
    const OFFSET_FILE: &'static str = "offset_file.offset";

    fn build_offset_file_path() -> PathBuf {
        let program_dir = std::env::current_dir().unwrap();
        return program_dir.join(TEST_PATH).join(OFFSET_FILE);
    }
    ...
    fn test_increment_offset() {
        let offset_path = build_offset_file_path();
        let mut offset_store = OffsetStore::init(&offset_path);
        assert!(offset_store.is_ok());

        let offset_store = offset_store.as_mut().unwrap();
        let res = offset_store.increment_offset();
        assert!(res.is_ok());
        assert_eq!(offset_store.offset(), 1);

        let offset_store2 = OffsetStore::init(&offset_path);
