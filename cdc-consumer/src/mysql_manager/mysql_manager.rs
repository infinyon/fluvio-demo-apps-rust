...
use cdc_messages::{FluvioMessage, Operation, Value};

use crate::cli::{Filters, Profile};
...
impl MysqlManager {
    ...
    pub fn update_database(&mut self, json_msg: String) -> Result<(), Error> {
        ...
        if !allowed_by_filters(&self.filters, &uri_props.db_name) {
            println!(
                ...
                uri_props.db_name.unwrap_or("".to_owned())
            );
            return Ok(());
        }
        println!("{:?}", &flv_message);
    ...
    fn send_query_op(&mut self, query: &String, uri_props: UriProps) -> Result<(), Error> {
...
fn exec_query(conn: &mut Conn, query: &String) -> Result<(), Error> {
    println!("[query] {:?}", query);
...
fn parse_uri(uri_str: &String) -> Result<UriProps, Error> {
    let uri = uri_str
        .parse::<Uri>()
        .map_err(|err| Error::new(ErrorKind::InvalidData, format!("{}", err)))?;
    let path: Vec<&str> = uri.path().split("/").collect();

    let db_name: Option<String> = if path.len() > 1 && path[1].len() > 0 {
        Some(path[1].to_owned())
    } else {
        None
    };

    let table_name: Option<String> = if path.len() > 2 && path[2].len() > 0 {
...
fn is_query_create_database(query: &String) -> bool {
...
mod test {
    use super::*;
    use cdc_messages::{BeforeAfterCols, Cols, Value, WriteRows};
